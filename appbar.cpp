        ...
        INT         nCmdShow)
{
    ...
    {
        MessageBox(NULL, TEXT("RegisterClassW failed"), s_szName, MB_ICONERROR);
        return -1;
    }

    HWND hwnd = Window::DoCreateMainWnd(hInstance, TEXT("Test1"), 100, 100);
    if (!hwnd)
    {
        MessageBox(NULL, TEXT("CreateWindowExW failed"), s_szName, MB_ICONERROR);
        return -2;
    }

    return Window::DoMainLoop();
}
